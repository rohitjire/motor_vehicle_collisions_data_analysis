...
injured_people = st.slider('Number of persons injured in vehical collisions', 0, 19)
...
st.markdown(f"Vehicle collisions between {hour % 24}:00 and{(hour + 1) % 24}:00")
...
st.subheader(f'Breakdown by minute {hour % 24}:00 and {(hour + 1) % 24}:00')
filtered = data[
